...
// Bit SPCR   Bit SPSR
// D1  D0     SPI2X
//  0   0     0           F_CPU/4   (4MHz   @16MHz)
//  0   0     1           F_CPU/2   (8MHz   @16MHz)
//  0   1     0           F_CPU/16  (1MHz   @16MHz)
//  0   1     1           F_CPU/8   (2MHz   @16MHz)
//  1   0     0           F_CPU/64  (250kHz @16MHz)
//  1   0     1           F_CPU/32  (500kHz @16MHz)
//  1   1     0           F_CPU/128 (125kHz @16MHz)
//  1   1     1           F_CPU/64  (250kHz @16MHz)
//
// Use SPI SCK multiply by 2 if nonzero
#define SPI_FCPU_X2   1
//------------------------------------------------------------------------------
...
static void spi_set_divisor(BYTE cardType) {
  if (!cardType) {
    ...
    SPCR = (1 << SPE) | (1 << MSTR) | 3;    // /128
    SPSR = 0;                               // *1 = /128 (125kHz)
  }  else {
    // Set high speed.
//  SPCR = (1 << SPE) | (1 << MSTR) | 1;    // /16
  SPCR = (1 << SPE) | (1 << MSTR);        // /4
  SPSR = SPI_FCPU_X2 ? 1 << SPI2X : 0;    // *1 / *2
  }
}
//------------------------------------------------------------------------------
